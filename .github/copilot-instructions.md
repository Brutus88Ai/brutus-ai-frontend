...
- Firestore listeners: `useFirestore` uses `onSnapshot` and correctly unsubscribes in cleanup — keep this pattern to avoid resource leaks.
...
- CSS: repo includes a minimal CSS fallback for builds without Tailwind; reintroducing Tailwind requires restoring `postcss.config.cjs` and devDependencies.
